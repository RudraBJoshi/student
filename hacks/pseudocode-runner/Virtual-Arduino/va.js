// VAX-126 — Virtual Arduino X · v1 · 2026
// Synchronous AST interpreter for PseudoSystem
// Usage: window.APCSP_VA.run(ast) → { frames, serial, error, steps }
//
//   frames : [{map, row, col, dir}, …]  — for tilemap animation
//   serial : ['line1', 'line2', …]       — Serial.println / DISPLAY output
//   error  : string | null
...
(function () {
  ...
  window.APCSP_VA = window.APCSP_VA || {};

  window.APCSP_VA.run = function (ast) {
    var MAX_STEPS = 5000;
    ...
    function evalBuiltin(name, argNodes) {
      ...
      switch (name) {
        ...
        case 'INPUT':
          throw new Error('INPUT() is not supported in Virtual Arduino mode');
      }
      return undefined;
    }
    ...
    function execStmt(s) {
      ...
      switch (s.type) {
        ...
        // Skip browser-only / storage ops silently
        case 'FromPkgImport':
